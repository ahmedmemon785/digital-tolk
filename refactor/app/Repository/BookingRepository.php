...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($currentUser && $currentUser->is('customer')) {
            ...
            //This function should also get called from the user model as $currentUser->getTranslatorJobs('new)

            $jobs = Job::getTranslatorJobs($currentUser->id, 'new');
            $jobs = $jobs->pluck('jobs')->all();//This step could be removed if the above function returns a collection
            $usertype = 'translator';
        }
        if ($jobs) {
...
//                }
//            }
            //This could be written in a more collection freindly way
            $jobGroups = $jobs->groupBy(function ($job) {
                return $job->immediate === 'yes' ? 'emergencyJobs' : 'normalJobs';
            });

            $emergencyJobs = $jobGroups->get('emergencyJobs', collect());
            $normalJobs = $jobGroups->get('normalJobs', collect());

            //So here we also don't need to convert to a collection

            $normalJobs = $normalJobs->map(function ($item) use ($user_id) {
...
//            })->sortBy('due')->all();
        ...
        return compact('emergencyJobs', 'normalJobs', 'currentUser', 'usertype');

//        return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $normalJobs, 'cuser' => $currentUser, 'usertype' => $usertype];
    }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        //Renamed this to a meaningful name
        $currentUser = User::find($user_id);
        $usertype = '';
        $emergencyJobs = [];
        $normalJobs = [];
        ...
        if ($currentUser && $currentUser->is('customer')) {
            $jobs = $currentUser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback', 'distance')
                ->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])
                ->orderBy('due', 'desc')->paginate(15);
            $usertype = 'customer';
            $numpages = 0;
            $pageNum = 0;
            return compact('emergencyJobs', 'normalJobs', 'jobs', 'currentUser', 'usertype', 'numpages', 'pageNum');
//            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => [], 'jobs' => $jobs, 'cuser' => $currentUser, 'usertype' => $usertype, 'numpages' => 0, 'pagenum' => 0];
        }

        if ($currentUser && $currentUser->is('translator')) {
            $pageNum = $page;
            $jobIds = Job::getTranslatorJobsHistoric($currentUser->id, 'historic', $pageNum);
...
//            $jobs['total'] = $totaljobs;
            return compact('emergencyJobs', 'normalJobs', 'jobs', 'currentUser', 'usertype', 'numpages', 'pageNum');
//            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $currentUser, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum];
        }
        return [];
    }
        ...
        try {
            ...
            if ($job->certified != null) {
                switch ($job->certified) {
                    case 'both':
                        array_push($data['job_for'], 'normal', 'certified');
                        break;
                    case 'yes':
            ...
            //This code has been commented as it is not required
//            $data['customer_town'] = $cuser->userMeta->city;
//            $data['customer_type'] = $cuser->userMeta->customer_type;

            //commented code should be deleted with specific reason why that code is not required to keep the code clean
...
//        $user = $job->user()->get()->first(); // This is a very bad way to do this. It is getting all users and then getting the first one.
            $user = User::findOrFail($job->user_id);//or we can modify the relation to return one user if there is only one job
            ...
            //most of the email sending data is fetched from user or job so there could be a model function for this calling sms by some service class
    ...
    {
        // Use Carbon for date and time operations
        ...
        // Use Carbon's createFromFormat to create date objects
        $start = Carbon::createFromFormat('Y-m-d H:i:s', $duedate);
        $end = $completeddate;

        // Calculate the time difference and format it as HH:MM:SS
        $diff = $end->diff($start);
...
// Send email with updated data
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        //This could be job->user->meta but it's not always the case
...
//        }
            ...
            //above step will always be true, so this is just a fallback
...
//            $translator_type = 'volunteer';
        ...
        $translator_type = $jobTypes[$job_type];
        ...
        if (!empty($job->certified)) {
            ...
            //This will never be true because empty is checked above
        ...
        //$job->blackListUsers(); It should be called from here
        $blacklist = UsersBlacklist::where('user_id', $job->user_id)->get();
        //this should be $blacklist->pluck('translator_id')->all();
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    {
        //this type of queries are more better in scope function
        //like $job->unCanclledTranslatorJobRel()->first();
        ...
        $job->save();
        //Below condition had same code in both if and else
//        if ($job->due <= Carbon::now()) {
//            $job->save();
//            //This is too bad approach of returning values
//            return ['Updated'];
//        } else {
        $job->save();
